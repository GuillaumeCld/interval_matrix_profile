        ...
        for (int metarow = 0; metarow < metarows; ++metarow)
        {
            ...
            for (int column = -1; column < nb_blocks; ++column)
            {
                ...
                for (int i = 0; i < block_height; ++i)
                {
                    ...
                    // if (block_j == 16422 and k==7 and block_i+i==13519 ) std::cout << "A min " << min_pair.value << " " << min_pair.index <<  " heap top " << local_heap.top().value << " Size " << local_heap_row.at(i).size() << std::endl;


                    if (min_pair.value < local_heap.top().value)
                    {
                        // if (k==7 and block_i+i==13519 ) std::cout << "B min " << min_pair.value << " " << min_pair.index <<  " heap top " << local_heap.top().value << " Size " << local_heap_row.at(i).size() << std::endl;
                        local_heap.pop();
                        local_heap.push(min_pair);
                        // if (k==7 and block_i+i==13519 ) std::cout << "B heap top " << local_heap.top().value << std::endl;
                    }
                }
            }
            // Compute the global minimums per row and update the matrix profile/index
